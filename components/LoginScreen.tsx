  ...
  return <main className="login-page">
    ...
    <section className="login-access">
      <div className="access-card">
        <div className="access-heading"><p className="login-kicker">Secure workspace</p><h2>{mode === "login" ? "Sign in to AssetFlow" : "Create your employee account"}</h2><p>{mode === "login" ? "Choose a demo profile, then enter its password to open the dashboard." : "New accounts are created as employees; protected roles are assigned by an administrator."}</p></div>
        {mode === "login" && <div className="demo-list" aria-label="Demo accounts">{demoAccounts.map((account) => <button type="button" className={selectedEmail === account.email ? "demo-account selected" : "demo-account"} key={account.email} onClick={() => selectAccount(account)}><span className="demo-avatar">{account.initials}</span><span><strong>{account.name}</strong><small>{account.role}</small></span><i>Use account</i></button>)}</div>}
        <form className="login-form" onSubmit={submit}>
          ...
          <div className="login-form-row"><label className="login-checkbox"><input type="checkbox" checked={remember} onChange={(event) => setRemember(event.target.checked)} /><span>Remember this device</span></label><button type="button" className="login-help">Need help?</button></div>
        ...
        {mode === "login" && <p className="demo-hint">Demo passwords: Hiten@2026, Sujith@2026, and Maha@2026.</p>}
