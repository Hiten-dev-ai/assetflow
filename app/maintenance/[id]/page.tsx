...
const actions: { label: string; value: MaintenanceAction }[] = [{ label: "Approve", value: "APPROVE" }, { label: "Reject", value: "REJECT" }, { label: "Assign technician", value: "ASSIGN_TECHNICIAN" }, { label: "Start work", value: "START_WORK" }, { label: "Resolve", value: "RESOLVE" }];
...
export default function MaintenanceDetailPage() {
  ...
  function act(action: MaintenanceAction) {
    try {
      ...
      setRequest(result.request); if (result.assetStatus) setAssetStatus(result.assetStatus); setMessage(result.notification.body);
    } catch (error) { setMessage(error instanceof MaintenanceRuleError ? error.message : "Action could not be completed."); }
  }
  ...
  return <FeatureShell title={`Maintenance request #${request.id}`}><section className="detail-grid"><article className="feature-panel"><div className="asset-summary"><span className="asset-icon">AV</span><div><p className="eyebrow">Room B2 · AF-0412</p><h2>Projector display issue</h2><p>{request.description}</p></div><span className="status-pill">Asset: {assetStatus.replaceAll("_", " ")}</span></div><h3 className="subheading">Workflow timeline</h3><ol className="workflow">{steps.map((step, index) => <li className={index <= stepIndex ? "done" : ""} key={step}><i /> <span><strong>{step.replaceAll("_", " ")}</strong><small>{index <= stepIndex ? "Completed" : "Waiting"}</small></span></li>)}</ol></article>
    <aside className="feature-panel action-card"><p className="eyebrow accent">Role-aware actions</p><h2>Update request</h2><label>Technician<input value={technician} onChange={(event) => setTechnician(event.target.value)} /></label><div className="action-stack">{actions.map((action) => <button key={action.value} onClick={() => act(action.value)}>{action.label}</button>)}</div><p className="form-message" role="status">{message}</p><small>Every change creates a notification and activity log entry.</small></aside></section></FeatureShell>;
}
